...
## Current status of all remaining ignored tests (as of 2026-03-30, session 88)
...
All 34 remaining harness tests were re-tested in session 88 — none have improved enough to pass. Session 88 fixed 19 BSIM3SOI-DD parameter defaults to match ngspice (Fix 88) but these don't affect any test because model cards specify all critical parameters. Also confirmed via comprehensive audit that DD parameter parsing is correct (111/112 params parsed, only PARAMCHK missing — zero simulation impact). The 0.63% DD t3 error and 0.385% VBIC FO error remain irreducible with current approaches; both are at the boundary of NR convergence tolerance and FP evaluation order differences.
