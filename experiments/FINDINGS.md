...
**Proven by `spike-ts-diff` (NESTED variant, 2026-07-12).** The spike now parses the
`code` cases on the snark side through a NESTED grammar (`grammar.nested.json`,
regenerated by the `nested-grammar` justfile recipe) while tree-sitter still uses the
real `scanner.c`. Measured:

- `code/brace_free` and `code/nested_braces` (`{ a:{ c } }`) → **agree node-for-node
  with scanner.c.** `nested_braces` *diverged* under the old no-scanner placeholder;
  raw NESTED now handles real balanced braces exactly. This is the whole result.
- `code/empty` (`code "x" {}`) → benign structural diff: NESTED spans the braces, so an
  empty block yields a `(code_body)` node where the external scanner emits none. Once
  cirq adopts NESTED this is just cirq's semantics (arguably cleaner) — not a bug.

**Out of scope (Alex's call):** a `}` hidden inside a string/comment (`const s = "} {"`)
— raw NESTED has no string awareness and closes early. Known, accepted limitation; the
long-term escape hatch (per Amos) is *not* a Rust scanner trait but a future
declarative scanner dialect that lowers to weavy/vix IR. Left out of the corpus.
