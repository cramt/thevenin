//! Runtime support the generated lowering calls into (`crate::support::*`).
//! Copied from vix's reference `support` module — the codegen emits the same
//! surface regardless of language.
#![allow(dead_code)]
...
/// A decoded leaf value plus where it came from.
...
/// Raw-text leaf (identifier, number, flag, token).
...
/// Token-valued fields scanned against a grammar-derived token set.
